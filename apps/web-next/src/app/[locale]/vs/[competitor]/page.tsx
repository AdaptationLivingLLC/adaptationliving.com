    ...
    <main id="main-content" className="navbar-offset">
      ...
      <section className="section-cream py-16 sm:py-20">
        <div className="mx-auto max-w-[1400px] px-6 sm:px-8">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h2 className="mt-3 h-display text-2xl font-bold text-[#0F172A] sm:text-3xl lg:text-[2.25rem]">
              Feature-by-feature, side by side.
            </h2>
            <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-[#475569]">
          ...
          <div className="mt-10 overflow-hidden rounded-[20px] border border-[#0F172A]/10 bg-white shadow-soft">
            <div className="grid grid-cols-[1.2fr_1fr_1fr] items-center gap-3 border-b border-[#0F172A]/10 bg-[#0F172A] px-4 py-4 text-white sm:px-6">
            ...
            {comparisonRows.map((row, i) => (
              <div
                key={row.feature}
                className={`grid grid-cols-[1.2fr_1fr_1fr] items-center gap-3 border-b border-[#0F172A]/5 px-4 py-3 sm:px-6 ${
                  i % 2 === 0 ? "bg-white" : "bg-[#0F172A]/[0.015]"
                }`}
              >
                <p className="text-sm font-semibold text-[#0F172A]">
                  {row.feature}
                </p>
                <p className="flex items-start gap-2 text-sm text-[#0F172A]">
                  <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-accent" />
                  <span>{row.ours}</span>
                </p>
                <p className="text-sm text-[#475569]">{row.theirs}</p>
              </div>
            ))}
          </div>

          <p className="mt-6 text-center text-xs uppercase tracking-[0.24em] text-[#0F172A]/50">
      ...
      <section className="section-cream py-16 sm:py-20">
        <div className="mx-auto max-w-[1400px] px-6 sm:px-8">
          <div className="max-w-3xl">
            <div className="mb-3 flex items-center gap-3">
              <MinusCircle className="h-6 w-6 text-[#0F172A]/50" />
              <p className="eyebrow">Where {c.name} May Be Stronger</p>
            </div>
            <h2 className="h-display text-2xl font-bold text-[#0F172A] sm:text-3xl lg:text-[2.25rem]">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {c.theirEdge.map((bullet) => (
              <div
                key={bullet}
                className="rounded-[18px] border border-[#0F172A]/10 bg-white p-5 shadow-soft"
              >
              ...
              {competitors
                ...
                .map((other) => (
                  <Link
                    key={other.slug}
                    href={`/vs/${other.slug}`}
                    className="rounded-full border border-white/20 bg-white/5 px-4 py-1.5 text-xs text-[#94A3B8] transition-colors hover:border-accent hover:text-accent"
                  >
                    vs {other.name}
                  </Link>
                ))}
